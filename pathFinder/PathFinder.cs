    ...
    public List<PathFinderNode> FindPath(Point start, Point end, bool _ignoreObstacle)
    {
        ...
        while(mOpen.Count > 0 && !mStop)
        {
            ...
            for (int i=0; i<(mDiagonals ? 8 : 4); i++)
            {
				...
				// Unbreakeable?
                if (mGrid[newNode.X, newNode.Y] == PathManager.GRID_VALUE_CANNOT)
                    continue;
				...
				if (_ignoreObstacle)
				{
					if (mHeavyDiagonals && i>3)
	                   	newG = parentNode.G + PathManager.GRID_VALUE_GOOD + PathManager.GRID_ADD_VALUE_DIAGONAL;
	                else
	                    newG = parentNode.G + PathManager.GRID_VALUE_GOOD;
				}
				else
				{
	                if (mHeavyDiagonals && i>3)
	                    //newG = parentNode.G + (int) (mGrid[newNode.X, newNode.Y] * 2.41);
						newG = parentNode.G + mGrid[newNode.X, newNode.Y] + PathManager.GRID_ADD_VALUE_DIAGONAL;
	                else
                ...
                switch(mFormula)
                {
                    ...
                    case HeuristicFormula.Manhattan:
						...
						newNode.H       = (Math.Abs(newNode.X - end.X) + Math.Abs(newNode.Y - end.Y)) << mHEstimate;
                        break;
